...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    // 'R' will propagate current 1-second sample value to entire baseline window
      ...
      // Check if we have enough samples for a valid 1-second average
      if (nBaselineSample > 0) {
        // Use current 1-second average
        currentValue = BaselineSumInWindow / (double)nBaselineSample;
        Serial.print("Using current 1-second average: ");
        Serial.println(currentValue, 2);
      } else {
        // If no baseline samples (e.g., in Idle state), use current signal reading
        currentValue = analogRead(InputPin);
        Serial.print("No baseline samples available. Using current signal: ");
        Serial.println(currentValue, 2);
      }
      
      // Fill entire baseline window with this value
      baseline_sum = 0;
      baseline_sum_squares = 0;
      
      for (int i = 0; i < BASELINE_WINDOW_SIZE; i++) {
        baselineWindow[i] = currentValue;
        baseline_sum += currentValue;
        baseline_sum_squares += currentValue * currentValue;
      }
      
      baselineWindowIndex = 0;
      baselineWindowCount = BASELINE_WINDOW_SIZE;
      baselineWindowFull = true;
